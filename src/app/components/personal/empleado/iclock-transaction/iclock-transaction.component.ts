...
export class IclockTransactionComponent implements OnInit {
  ...
  tieneHorario(transactionComplete: TransactionComplete): boolean {
    // verificamos que transactionComplete.turnoAlias no sea undefined y que no tenga 'Sin horario'
    if (transactionComplete.turnoAlias === undefined || transactionComplete.horarioEntrada === '00:00:00' && transactionComplete.horarioSalida === '00:00:00') {
  ...
  abrirModalMarcacion(tipo: 'entrada' | 'salida'): void {
    ...
    if (!this.selectedMarcacion || !this.tieneHorario(this.selectedMarcacion)) {
      this.showToast('info', 'Información', 'No tiene horario asignado.');
      return;
    }
    console.log("ddddd",this.selectedMarcacion);
